...
import { TModel, TOptions, TModelField } from '../../../../types';
...
export class ModelField {
    ...
    this.mercury.hook.before(
      ...
      async function (this: any) {
        if (this.record.managed)
          throw new Error(`This model field can't be edited!`);
      }
    );
    this.mercury.hook.after(
      'UPDATE_MODELFIELD_RECORD',
    ...
    this.mercury.hook.before(
      ...
      async function (this: any) {
        ...
        if (modelFields.length == 1)
          throw new Error('Model has only one field thus it cant be deleted');
        if (this.record.managed)
          throw new Error(`This model field can't be deleted!`);
      }
    );
    this.mercury.hook.after(
      'DELETE_MODELFIELD_RECORD',
  ...
  @AfterHook
  private async syncModelFields(
    modelField: TModelField,
    prevRecord?: TModelField
  ) {
    let redisObj: TModel = JSON.parse(
      (await this.mercury.cache.get(
        modelField.modelName.toUpperCase()
      )) as string
    );
    ...
    this.mercury.createModel(redisObj.name, redisObj.fields, {
      ...redisObj.options,
      update: redisObj.name in this.mercury.db,
    } as TOptions);
  }
