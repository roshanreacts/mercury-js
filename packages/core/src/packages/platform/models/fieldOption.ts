...
import { TFieldOption, TModel, TOptions } from '../../../../types';
...
export class FieldOption {
    ...
    this.mercury.hook.before(
      ...
      async function (this: any) {
        if (this.record.managed)
          throw new Error(`This field option  can't be deleted!`);
      }
    );
  ...
  private updateFieldOptionHook() {
    ...
    this.mercury.hook.before(
      ...
      async function (this: any) {
        if (this.record.managed)
          throw new Error(`This field option can't be edited!`);
      }
    );
    this.mercury.hook.after(
      'UPDATE_FIELDOPTION_RECORD',
  ...
  private createFieldOptionHook() {
    ...
    this.mercury.hook.before(
      ...
      async function (this: any) {
        ...
        const modelField = await _self.mercury.db.ModelField.get(
          { _id: this.data.modelField },
          { id: '1', profile: 'SystemAdmin' }
        );
        if (model.name !== this.data.modelName)
          throw new Error('Model name mismatch !!');
        if (modelField.model.toString() !== this.data.model)
          throw new Error("Model Field doesn't belongs to this model!!");
        if (modelField.fieldName !== this.data.fieldName)
    ...
    this.mercury.hook.after(
      ...
      async function (this: any) {
        if (this.options.skipHook) return;
        const fieldOption = await _self.mercury.db.FieldOption.get(
          { _id: this.record._id },
          { id: '1', profile: 'SystemAdmin' }
        );
