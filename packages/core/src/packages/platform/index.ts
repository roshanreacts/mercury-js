...
export class Platform {
	...
	public composeSchema(modelFields: TModelField[], fieldOptions?: TFieldOption[]) {
		const skipFields = [
			'id',
			'_id',
			'type',
	...
	public composeOptions(modelOptions: TModelOption[]) {
		let options: TOptions = { historyTracking: false, private: false};
	...
	private async subscribeToModelHooks() {
		...
		this.mercury.hook.before('CREATE_MODEL', async function (this: TModel) {
			if (
				...
				return;
			let redisObj: string | TModel | null = await _self.mercury.cache.get(
				this.name.toUpperCase()
			);
			if(redisObj != null)
			redisObj = JSON.parse(redisObj) as TModel;
